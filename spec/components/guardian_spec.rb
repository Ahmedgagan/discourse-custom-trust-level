...
describe Guardian do
  ...
  describe "can_create_post_on_topic?" do
    ...
    it 'returns false if users have posts count equal to site setting csl_numbers_of_replies_upto_20_posts' do
      ...
      expect(Guardian.new(trust_level_1).can_create_post_on_topic?(topic)).to be_falsey
    end
    ...
    it 'returns true if user is admin' do
      expect(Guardian.new(admin).can_create_post_on_topic?(topic)).to be_truthy
    end

    it 'returns true if user is admin' do
      expect(Guardian.new(moderator).can_create_post_on_topic?(topic)).to be_truthy
    end
  end
